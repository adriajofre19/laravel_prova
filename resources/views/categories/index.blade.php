                        ...
                        <table class="min-w-full bg-white">
                            ...
                            <tbody>
                                @foreach($categories as $category)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap border-b border-gray-300">{{ $category->nombre }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap border-b border-gray-300">{{ $category->descripcion }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap border-b border-gray-300">
                                            <a href="{{ route('categories.edit', $category) }}" class="text-blue-600 hover:text-blue-900 mr-3">Editar</a>
                                            <form action="{{ route('categories.destroy', $category) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('¿Estás seguro de eliminar esta categoría?')">Eliminar</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
